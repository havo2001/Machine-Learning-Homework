...
class DecisionTree(BaseEstimator):
    ...
    def choose_best_split(self, X_subset, y_subset):
        ...
        for current_feature in range(X_subset.shape[1]):
            for current_threshold in np.unique(X_subset[:, current_feature]):
    ...
    def make_tree(self, X_subset, y_subset, depth):
        ...
        if depth < self.max_depth and y_subset.shape[0] > self.min_samples_split:
            ...
            new_node = Node(feature_index=feature_index, value=threshold)
            ...
            if self.classification:
                if y_subset.shape[0] != 0:
                    prob = np.sum(y_subset, axis=0) / y_subset.shape[0]
                else:
                    prob = 0
                new_node.proba = prob
            else:
    ...
    def predict(self, X):
        ...
        if self.classification:
            ...
            q = queue.Queue()
            q.put(Vertex(self.root, X, index))
            while not q.empty():
                vertex = q.get()
                ...
                if current_node.left_child is None and current_node.right_child is None:
                    ...
                    q.put(Vertex(current_node.left_child, X_left, y_left))
                    q.put(Vertex(current_node.right_child, X_right, y_right))
